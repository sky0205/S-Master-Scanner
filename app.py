...
# 1. 화면 구성 및 할배 캐릭터 스타일 (이미지 양식 완벽 복구)
st.set_page_config(page_title="S-Master-Scanner v36061", layout="wide")
st.markdown("""
    <style>
...
if symbol:
    try:
        ...
        if not df.empty:
            ...
            st.markdown(f"<div class='vol-box'><div style='font-size:32px; font-weight:bold; color:#0D47A1; margin-bottom:12px;'>📊 거래량 전황: {'매집중' if v_ratio>=100 else '기세부족'} ({v_ratio:.1f}%)</div><div class='vol-sub-text'>✅ 현재 거래율 {v_ratio:.1f}%로 {'추세를 타시게.' if v_ratio>=100 else '아군 화력을 기다리시게.'}</div></div>", unsafe_allow_html=True)

            if p >= up_b or p_chg > 5: sig, col, s_adv = "🟢 매도권 진입", "#388E3C", "● 익절하시게."
            elif p <= low_b: sig, col, s_adv = "🔴 매수권 진입", "#D32F2F", "● 보따리 푸시게."
            ...
            with c1: st.markdown(f"<div class='price-card'><p>⚖️ 공략 대기선</p><p style='color:#388E3C; font-size:45px;'>{format(low_b, fmt_p)}</p></div>", unsafe_allow_html=True)
            with c2: st.markdown(f"<div class='price-card'><p>🎯 수확 목표선</p><p style='color:#D32F2F; font-size:45px;'>{format(up_b, fmt_p)}</p></div>", unsafe_allow_html=True)
            with c3: st.markdown(f"<div class='price-card'><p>🛡️ 성벽(방어선)</p><p style='color:#E65100; font-size:45px;'>{format(defense_line, fmt_p)}</p></div>", unsafe_allow_html=True)

            st.markdown(f"<div class='trend-card'><div class='trend-title'>⚔️ {name} 실전 필살 대응 전략</div><div class='trend-item'>1. **성벽 사수:** 현재가 {p:{fmt_p}}원, 성벽 {defense_line:{fmt_p}}원 {'위' if p>defense_line else '아래'}.</div><hr><div class='trend-item' style='color:#D32F2F; font-size:28px !important;'>🎯 **최종 결론:** {'분할 수확!' if p>=up_b else '매복 관망!'}</div></div>", unsafe_allow_html=True)
            ...
            with i1: st.markdown(f"<div class='ind-box'><p class='ind-title'>Bollinger (기세)</p><p class='ind-diag'>{'⚠️ 과열' if p>=up_b else '🏰 성벽사수'}</p></div>", unsafe_allow_html=True)
            with i2: st.markdown(f"<div class='ind-box'><p class='ind-title'>RSI (온도)</p><p style='font-size:45px; color:#E65100;'>{rsi_val:.2f}</p><p class='ind-diag'>{'👺 불지옥' if rsi_val>=60 else '중립'}</p></div>", unsafe_allow_html=True)
            ...
            with i4: st.markdown(f"<div class='ind-box'><p class='ind-title'>MACD (엔진)</p><p class='ind-diag'>{'● 정회전' if m_l>s_l else '● 역회전'}</p></div>", unsafe_allow_html=True)
